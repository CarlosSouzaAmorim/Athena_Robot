...
get_system_info() {
    ...
    if command -v free >/dev/null 2>&1; then
        ...
        echo "RAM: Total: $RAM_TOTAL, Usada: $RAM_USED, Disponível: $RAM_AVAILABLE"
    else
        echo "RAM: comando 'free' não disponível"
    fi
    
    # Espaço em disco
    if command -v df >/dev/null 2>&1; then
        echo "Disco:"
        df -h / | awk 'NR==2 {print "  Total: "$2", Usado: "$3", Livre: "$4" ("$5" usado)"}'
        if [ -d "$HOME" ]; then
            echo "  Home ($HOME):"
            df -h "$HOME" | awk 'NR==2 {print "    Total: "$2", Usado: "$3", Livre: "$4" ("$5" usado)"}'
        fi
    fi
    
    # Verificar se Ollama está instalado e obter modelos instalados
    if command -v ollama >/dev/null 2>&1; then
        echo ""
        echo "Ollama: instalado"
        # Tenta obter lista de modelos instalados (pode falhar se servidor não estiver rodando)
        if start_ollama_server >/dev/null 2>&1; then
            INSTALLED_MODELS=$(ollama list 2>/dev/null | awk 'NR>1 {print $1}' | tr '\n' ' ')
            if [ -n "$INSTALLED_MODELS" ]; then
                echo "Modelos instalados: $INSTALLED_MODELS"
            else
                echo "Modelos instalados: nenhum"
            fi
        else
            echo "Modelos instalados: servidor Ollama não disponível para verificar"
        fi
    else
        echo ""
...
select_model() {
    ...
    for model in "${!OLLAMA_MODELS[@]}"; do
        ...
        if echo "$installed_models" | grep -q "^$model$"; then
            installed_marker=" ✓ INSTALADO"
        fi
        
        echo "$i. ${OLLAMA_MODELS[$model]}${installed_marker}"
        ((i++))
    done
    
    echo ""
    echo "Legenda: ✓ INSTALADO = modelo já está instalado no sistema"
    echo ""
    ...
    if [[ "$choice" =~ ^[0-9]+$ ]] && [[ "$choice" -ge 1 ]] && [[ "$choice" -le "${#model_names[@]}" ]]; then
        ...
        if echo "$installed_models" | grep -q "^$SELECTED_MODEL$"; then
            echo ""
            echo "⚠️  Modelo '$SELECTED_MODEL' já está instalado."
            if confirm "Deseja reinstalar/atualizar este modelo?"; then
                echo "Reinstalando modelo $SELECTED_MODEL..."
            else
                echo "Mantendo versão atual do modelo."
                return 2
            fi
        else
...
# Mostrar informações do sistema primeiro
get_system_info
...
# Atualizar informações do sistema após instalação do Ollama
echo ""
get_system_info
...
if [ "$HAVE_INTERNET" -eq 1 ] && command -v ollama >/dev/null 2>&1; then
    if select_model; then
        if [ "$?" -eq 2 ]; then
            # Modelo já instalado e usuário optou por não reinstalar
